...
dataset_train = pd.read_csv("resources/NIFTY_BANK.csv")
#training_set = dataset_train.loc[:, ["Open Price","Prev Close","Total Traded Quantity","Close Price"]].values
training_set = dataset_train.loc[:, ["Open","Close","High","Adj Close"]].values
#Feature Scaling
...
#first layer
regressor.add(CuDNNLSTM(units = 50, return_sequences = True, input_shape = (x_train.shape[1], 4)))
...
#fourth layer
regressor.add(CuDNNLSTM(units = 25)) # return sequences will be false here since this is the last LSTM layers
regressor.add(Dropout(0.2))
...
dataset_total = dataset_train.loc[:, ["Open","Close","High","Adj Close"]]
